...
df_behaviour_raw <- read.csv('behavioural_test_data.csv')
...
for (num in 1:16)   ###Derive forrest from trial_nr, required for payoff calculation
  {
    row_index <- (num-1) * 50
df_behaviour[(row_index + 1):(row_index+50),1:7] <- (df_behaviour_raw[
                      c(
  ...
  df_behaviour[(row_index + 1):(row_index+50),"forrest_nr"] <- ((df_behaviour_raw[paste(c("foraging_simulation.", num, ".player.trial_in_game"), collapse = "")])+3)   %/% 4
}
...
for (treatment in list('joy','control'))
{
  ...
  if (best_model_one_nr == 1){
    ...
    best_model_nr <- (which.min(c(BIC(best_model_one), BIC(model_optimal_gain), BIC(model_optimal-threat))))
